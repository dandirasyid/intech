    ...
    <div class="container">
        <div class="bg-body my-4 p-3 border-0 rounded shadow">
            <div class="d-flex justify-content-center align-items-center mb-1 col-md-12">
                <form action="{{ route('admin_matakuliah.update', $matakuliah->id) }}" method="POST" enctype="multipart/form-data" class="col-md-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group mb-3 d-flex justify-content-between">
                        ...
                        <div style="width: 70%;">
                            <input type="text" name="name" id="name" class="form-control rounded-3 @error('name') is-invalid @enderror" placeholder="Masukan nama matakuliah" value="{{ $matakuliah->name }}">
                        </div>
                    </div>
                    @error('name')
                    ...
                    <div class="form-group d-flex mb-3 justify-content-between">
                        ...
                        <div style="width: 70%;">
                            <textarea class="form-control @error('deskripsi') is-invalid @enderror" name="deskripsi" id="deskripsi" rows="3" placeholder="Masukan deskripsi">{{ $matakuliah->deskripsi }}</textarea>
                        </div>
                    </div>
                    @error('deskripsi')
                    <div class="text-danger mt-2">{{ $message }}</div>
                    ...
                    <div class="form-group d-flex mb-3 justify-content-between">
                        ...
                        <div style="width: 70%;">
                            <input type="text" name="semester" id="semester" class="form-control rounded-3 @error('semester') is-invalid @enderror" placeholder="Contoh: 1" value="{{ $matakuliah->semester }}">
                        </div>
                    </div>
                    @error('semester')
                    <div class="text-danger mt-2">{{ $message }}</div>
