    ...
    <div class="container">
        <div class="bg-body my-4 p-3 border-0 rounded shadow">
            <div class="d-flex justify-content-center align-items-center mb-1 col-md-12">
                ...
                @endif
                <form action="{{ route('update_tugas', ['matakuliah_id' => $matakuliah->id, 'tugas_id' => $tugas->id]) }}" method="POST" enctype="multipart/form-data" class="col-md-8 my-5">
                    @csrf
                    @method('PUT')
                    <div class="form-group mb-3">
                        <h5 class="text-center fw-bold ">Edit Tugas {{$matakuliah->name}}</h5>
                    </div>

                    <div class="form-group upload">
                        <img src="{{ $tugas->image ? Storage::url($tugas->image) : asset('images/edit-profile.png') }}" id="profileImage" class="rounded-circle mb-2" alt="logo-medsos" width="100px" height="85px">
                    ...
                    <div class="form-group mb-3 d-flex justify-content-between">
                        ...
                        <div style="width: 70%;">
                            <input type="text" name="judul" id="judul" class="form-control rounded-3 @error('judul') is-invalid @enderror" placeholder="Masukan judul materi" value="{{ $tugas->judul }}">
                        </div>
                    </div>
                    @error('judul')
                    ...
                    <div class="form-group mb-3 d-flex justify-content-between">
                        ...
                        <div style="width: 70%;">
                            <input type="datetime-local" name="deadline" id="deadline" class="form-control rounded-3 @error('deadline') is-invalid @enderror" value="{{ \Carbon\Carbon::parse($tugas->deadline)->format('Y-m-d\TH:i') }}">
                        </div>
                    </div>
                    @error('deadline')
                    <div class="text-danger mt-2">{{ $message }}</div>
                    ...
                    <div class="form-group mb-3 d-flex justify-content-between">
                        ...
                        <div style="width: 70%;">
                            <textarea class="form-control @error('deskripsi') is-invalid @enderror" name="deskripsi" id="deskripsi" rows="3" placeholder="Masukan deskripsi">{{ $tugas->deskripsi }}</textarea>
                        </div>
                    </div>
                    @error('deskripsi')
                    <div class="text-danger mt-2">{{ $message }}</div>
